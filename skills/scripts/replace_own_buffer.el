(macher-agent-make-tool macher-agent-replace-own-buffer-tool
  ...
  :success-fn (lambda (raw-buf-name _payload)
                (format "SYSTEM: Operation acknowledged for buffer '%s'. Your buffer will be overwritten immediately upon turn completion." 
                        raw-buf-name)))
