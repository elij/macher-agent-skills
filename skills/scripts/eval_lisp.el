 ...
 (lambda (payload _context _root)
   (let*
       ((code (plist-get payload :code))
        (expression
         (condition-case
             nil (car (read-from-string (format "(progn %s)" code)))
           (error nil)))
        (result
         (if
             expression
             (condition-case err
                 (let
                     ((res
                       (macher-agent-sandbox-run
                        expression
                        '(cons list string-to-list substring emacs-version
                               ...
                               not identity reverse nreverse sort delete delq
                               nconc assoc assq rassoc rassq copy-sequence
                               split-string replace-regexp-in-string upcase
                               downcase capitalize plist-get plist-put
                               plist-member error signal make-symbol intern
                               intern-soft match-string  match-beginning
                               match-end make-string string char-to-string
                               aset make-hash-table puthash gethash
