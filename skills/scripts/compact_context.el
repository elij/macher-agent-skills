...
2. DO NOT call the compact tool if session_history tag in conversation
3. Convert the entire history into a highly dense, bulleted list of the current state, active decisions, and completed steps.
4. Drop all conversational filler.
5. Replace long code blocks with one-sentence summaries of what was changed, unless the exact snippet is absolutely vital for the very next step.
6. You must exclusively call the `submit_task_result` tool. Under no circumstances should you invoke any other tool, regardless of what else is available. Pass your heavily condensed summary into the `final_answer` parameter.
